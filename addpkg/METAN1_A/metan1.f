      SUBROUTINE METAN1 (N,FCN,T,Y,TEND,TOL,HMAX,H,KFLAG,SOUT)
C
...
C
     ...
     6H, HALF, HJ, HJA, HL, HMAX, HMAXMY, HMAXT, HMAXU, HN, HREST,HRTRN,
     7M1ERRN, OMJ, OMJO, ONE, PCT1, QQ, RED, RMAX, RHO, ROW, SAFE,
     8SAFEIN, SD, SMALL, SUMD, T, TAUD, TAUQ, TEND, THETA, THIRD, THMAX,
     9THMIN, THOPT, THQ, TN, TOLD, TOL, TOLH, TOLMIN, TOLN, TWO, U, V,
     AW, WY, WZ, XQ1, XQ2, Y, YA, YM, YQ1, YQ2, YWGT, YWGTN,
     BZERO, ZQ1, ZQ2
C
      LOGICAL QDM, QDMA, QINCR, QJACNW, QJACUS, QKONV, QMY, QOPEN,
     2QPRMON, QPROUT, QPRSOL, QRED, QTEST
C
...
C     QPROUT   Use callback routine SOUT (supplied by the user)
C
C* Dimensions:
C
...
C---1. Initial preparations
      ...
      QPRMON = (KFLAG .EQ. 1 .OR. KFLAG .EQ. 3 .OR. KFLAG .EQ. 5)
      QPRSOL = (KFLAG .EQ. 2 .OR. KFLAG .EQ. 3)
      QPROUT = (KFLAG .GE. 4)
      IF (TEND .LT. T) THEN
C        Error 1
