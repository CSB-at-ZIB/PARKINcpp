...
c                         4.3B (sparse Jacobians).
c
c                Iopt(31) Dierkes special option no. 1:
c                         0 : B is the n x n identity matrix
c                         1 : B is constant
c                         2 : B may be not constant or depend on y
c
c                Iopt(32) Dierkes special option no. 2:
c                         If Iopt(32) = 1, the intermediate solutions,
c                         the stepsize  and  the order of  the current
c                         step will be stored in the array Dense.
c
...
c-----------------------------------------------------------------------
c
      integer            n, kOrder, Iopt ( * ), IPos ( * ), IFail ( 3 )
c
...
c
      double precision   CStep, ex1, ex2, fmin, h_Min, one, one1, quart,
     2                   RedFcn, RedPos, rmin, ro, safe, Small, ten,
     3                   ThMin, Thresh, zero
c
      parameter ( CStep  = 1.0d-7,  ex1    = 0.6d0 , ex2    = 1.5d0 ,
     2            fmin   = 1.0d-2, h_Min  = 1.0d-4 , one    = 1.0d0 ,
...
c     the one step mode or for continuation calls. Then many variables
c     must retain their values after a return from LIMEX.
c
c-----------------------------------------------------------------------
c
      save
...
cTD
      if ( Iopt(31) .ne. 0 .and. nz .gt. Max_Non_Zeros_B ) go to 380
c
...
c
  140 continue
c
      if ( Iopt(7) .eq. 0 .or. iCIV .eq. 1 ) then
c
...
c
cTD
      if ( n_Dense .eq. 0 .and. Iopt(32) .eq. 1 ) then
c
