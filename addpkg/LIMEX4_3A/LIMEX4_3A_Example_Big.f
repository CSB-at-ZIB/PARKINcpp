...
c     of DAE-systems with LIMEX version 4.3A.
c
...
c
c     Iopt(19) - Iopt(23) 
c              not used in LIMEX version 4.3A
c
...
c
c     Ropt(4), Ropt(5) 
c              not used in LIMEX versions 4.3A
c
c-----------------------------------------------------------------------
c
      Iopt(1)  = 2
...
c
         ...
         xk_s    = rpar(98)
c
         hilfa =   log ( r_i / r_b ) / ( 2.0d0 * pi * xk_s * xl )
     2           + log ( ( r_i + d_bm ) / r_i ) 
     3                 / (2.0d0 * pi * xlam_bm * xl )
     4           + log ( ( r_a + r_i ) / ( 2.0d0 * r_i ) ) 
     5                 / ( 2.0d0 * pi *xlam_w * xl )
c
         hilfb =   1.0d0 / ( 4.0d0 * pi * alpha_a * r_a * r_a )
     2           + log ( 20d0 * r_a / ( r_a + r_i ) )
     3                / ( 2.0d0 * pi * xlam_w * xl )
c
         xk_u = 1.0d0 / ( hilfb * a_b )
         xk_i = 1.0d0 / ( hilfa * a_b )
c
         xfakhemm = rpar(28)
         xhoch    = rpar(27)
         xfako2   = rpar(26)
...
c
      xxkz_co    = xkz_co * exp ( - ez_co / ti )
      xxkn_co    = xkn_co * exp (   en_co / ti )
      xxkz_c3h6  = xkz_c3h6 * exp ( - ez_c3h6 / ti )
      xxkn_c3h6  = xkn_c3h6 * exp (   en_c3h6 / ti )
      xxk_coc3h6 = xk_coc3h6 * exp ( e_coc3h6 / ti )
c
c-----------------------------------------------------------------------
c
      g1co = 1.0d0 + xfakhemm * xxkn_co * abs ( y_cos )
      g1hc = 1.0d0 + xfakhemm * xxkn_c3h6 * abs ( y_c3h6s )
c
      r_co   = xxkz_co * y_cos * y_o2s / ( ti * g1co ** xhoch )
      r_c3h6 = xxkz_c3h6 * y_c3h6s * y_o2s / ( ti * g1hc ** xhoch )
      r_h2   = xxkz_co * y_h2s * y_o2s / ( ti * g1co ** xhoch )
      r_o2   = 0.5d0 * r_co + 4.5d0 * r_c3h6 + 0.5d0 * r_h2
      r_o2   = xfako2 * r_o2
c
      q(1)  =   xk_i / r_i * ( t_kat - t_wand )
     2        - xk_u * r_a * ( t_wand - tu ) / ( r_i * r_i ) 
      q(2)  = alpha_i * av * ( t_kat - t_gas )
      q(3)  = - xk_i / r_i * ( t_kat - t_wand )
     2        - alpha_i * av * ( t_kat - t_gas )
     3        + xhr_co * ax * 10.0d0 * r_co
     4        + xhr_c3h6 * ax * 10.0d0 * r_c3h6
     5        + xhr_h2 * ax * 10.0d0 * r_h2
      q(4)  =   rho_g * beta_co * av * ( w_cog - w_cos )
     2        - ax * 10.0d0 * xm_co * r_co
      q(5)  = - rho_g * beta_co * av * ( w_cog - w_cos )
      q(6)  =   rho_g * beta_c3h6 * av * ( w_c3h6g - w_c3h6s )
     2        - ax * 10.0d0 * xm_c3h6 * r_c3h6
      q(7)  = - rho_g * beta_c3h6 * av * ( w_c3h6g - w_c3h6s )
      q(8)  =   rho_g * beta_h2 * av * ( w_h2g - w_h2s )
     2        - ax * 10.0d0 * xm_h2 * r_h2
      q(9)  = - rho_g * beta_h2 * av * ( w_h2g - w_h2s )
      q(10) =   rho_g * beta_o2 * av * ( w_o2g - w_o2s )
     2        - ax * 10.0d0 * xm_o2 * r_o2
...
c
      t_steig = rpar(95)
      t_zu = t * ( temp_ende - temp_anfang ) / t_steig + temp_anfang
c
      if ( t. gt. 100.0d0 ) t_zu = temp_ende
c
c-----------------------------------------------------------------------
c
      alphal(1,1) = 0.0d0
