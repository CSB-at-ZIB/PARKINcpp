...
c
c     Max_Size   Integer variable, must be set by caller on entry, not
c                modified. Leading  dimension of  the  array  Dense as
c                declared in LIMEX.
c
c     Dense      Real array of size at least Max_Size * ipt(k+1), must
c                be set by caller on entry. The values of intermediate
...
c
c     k          Integer variable, must be set by caller on entry. The
c                currrent order of the extrapolation scheme.
c
c     ipt        Integer  array of size at least k + 1, must be set by
c                caller on  entry. Pointer array  to the  intermediate
c                solutions.
c
c     nj         Integer array  of size at least k + 1, must be set by
c                caller on entry. The stepsize dividing sequence.
c
c     Work       Real  array of size  at least n * ( k + 1 ), need not
c                be set by caller on entry. Work array.
c
c-----------------------------------------------------------------------
c
...
c                set by caller on entry. The coefficients of a Hermite
c                interpolation poynomial.
c
c     k          Integer variable, must be set by caller on entry. The
c                currrent order of the extrapolation scheme.
c
c     tFac       Real  variable, must  be set  by caller on entry. The
...
c                entry. On  exit the estimated  solution values at the
...
c                modified. Size of the system.
c
c     LenJac     Integer variable, must be set by caller on entry, not
...
c                on  entry only  if  mode = 0 or 1, not  modified. The
c                values of the Jacobian matrix in full or banded form.
c
c     LDJac      Integer variable, must be set by caller on entry only
c                if mode = 0 or 1, not  modified. Leading dimension of
c                array Jac.
c
c     ml         Integer variable, must be set by caller on entry only
c                if mode = 1, not  modified. For  banded Jacobians the
c                number of lower diagonals.
c
c     mu         Integer variable, must be set by caller on entry only
c                if mode = 1, not  modified. For  banded Jacobians the
c                number of upper diagonals.
c
c     ir         Integer array, must be set by caller on entry only if
c                mode = 2, not modified. Row  indices for the non-zero
c                entries of the Jacobian in sparse form.
c
c     ic         Integer array, must be set by caller on entry only if
c                mode = 2, not  modified. Column indices for the  non-
c                zero entries of the Jacobian is sparse form.
c
c     nStep      Integer variable, must be set by caller on entry, not
c                modified. Current step number, which  is displayed in
c                the title.
c
c     mode       Integer variable, must be set by caller on entry, not
...
c
      write ( 99, 9000 ) '%!'
      write ( 99, 9000 ) '%%Creator: JacPlot in LIMEX4.3'
     ...
     2   '/Helvetica findfont 0.5 cm scalefont setfont'
      ...
      write ( 99, 9030 ) '(Jacobian matrix from LIMEX4.3, step = ',
     2   nStep, ') Cshow'
